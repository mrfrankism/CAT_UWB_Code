...
Converter(String f){
  ...
  public void run(){//used to convert files in the background on a separate thread after scanning is done
 ...
 while(input.available() > 0){
  ...
  if (messageIndex + 1 == totalMessageNumber  && totalMessageNumber != 0) { // to stop recording the previous scan and start recording a new one when the last message has bee recieved for the previous scan
    ...
    ScanningFromManyReceivers.log.append("File not found trying to create CSVs");
  }
  catch(NullPointerException e){
   ScanningFromManyReceivers.log.append("NULLPOINTER trying to create CSVs"); 
  }
  catch(IOException e) {
    ScanningFromManyReceivers.log.append("IOException");
  }
  ScanningFromManyReceivers.log.append("Bytes skipped: " + skipByteCounter);
  scanNumber = 0;
 }
 ...
 public void writeScanData(int p) {
  ...
  if (writerExists == false) {
    try {
      String title = filePaths[p].substring(0,filePaths[p].indexOf('.'));
...
if (messageIndex + 1 == totalMessageNumber  && totalMessageNumber != 0) {
   for (int i = previousLength; i < scanData.size()-2; i++) {//skips the first index because it is a zero used to create the array
    writer.print(scanData.get(i)+", ");
  }
  writer.println(scanData.get(scanData.size()-1));
}else{
  for (int i = previousLength; i < scanData.size(); i++) {//skips the first index because it is a zero used to create the array
    writer.print(scanData.get(i)+", ");
  }
}
  previousLength = scanData.size();
  writer.flush();
}
...
public void storeScanData() {
  if (messageIndex == 0) {//start of a new message
    scanData = new ArrayList<Integer>(1);
  }
  for (int i = 0; i < parameters.get ("Number of Samples"); i++) {
    scanData.add(readWaveformData());
  }
  //println(scanData);
}
...
public int readWaveformData() { //reads 4 bytes and concatinates them into an int
  try {
    ...
    ScanningFromManyReceivers.log.append("File Not Found");
  }
  catch(IOException e) {
    ScanningFromManyReceivers.log.append("IOEXCEPTIO");
  }
  return -1;
}
